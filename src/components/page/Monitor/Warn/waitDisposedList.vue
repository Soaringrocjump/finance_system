...
<script>
    import {getDictionary} from '../../../../components/common/codes';
     var departmentUp;
export default {
  data () {
    return {
  ...
  methods: {
    ...
    //         ? result.data.data
    //         : "";
    //       //sessionStorage.setItem("departmentUp", JSON.stringify(msg.values));
    //       departmentUp=msg
    //     })
    //     .catch(err => {
    //       alert("错误：获取数据异常" + err);
    ...
    /*判断有没有上级部门的缓存*/
   /* hasDepartment(){
        departmentUp = JSON.parse(sessionStorage.getItem('departmentUp'));
        if (departmentUp=='' || departmentUp==null) {
            this.getCode();
            departmentUp=JSON.parse(sessionStorage.getItem('departmentUp'));
            return departmentUp;
         ...
         // return this.$common.getValue(departmentUp,38);
          return departmentUp;
        }
    },*/

    departFormat(row, column) {
      //this.$nextTick(()=>{
     //   return this.$common.getValue(departmentUp,38)
      //})
