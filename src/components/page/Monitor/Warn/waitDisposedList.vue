    ...
    <div class="Listbackground">
      ...
      <el-table :data="warningList" style="width: 100%" :header-cell-style="headerCellStyle" :row-style="rowStyle" :cell-style="cellStyle" :default-sort="{prop: 'date', order: 'descending'}">
        ...
        <el-table-column
          ...
          min-width="100">
          <template slot-scope="scope">
            <el-tag :type="scope.row.high_unit === '单位1' ? 'primary' : 'success'" disable-transitions>
              {{scope.row.high_unit}}
            </el-tag>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getCode(currentPage, warnStatus){
    ...
    toDetails(row){
      console.log(row);
      //这里换成详情页路由
      this.$router.push({ path: '/disposedDetail', query: { userId: row.uuid }});
    },
  },
  mounted(){
    this.getDataList();
