...
<template>
  <div class="dialogBody">
			<el-dialog title="处置" :visible.sync="dialogVisible" width="40%" center :show-close="false" :modal-append-to-body="false" :close-on-click-modal="false">
				<dl >
            ...
            :on-success="getFileId">
				...
				<span slot="footer" class="dialog-footer">
					...
					<el-button class="confirm" type="primary" @click="launchCheck">发起处置</el-button>
				</span>
			</el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    launchCheck(){
      ...
      this.$axios({
        method: "post",
        url: this.HOME +"warning/deal?mainDeptIds="+this.checkDepGroup1.join()+"&duplicateDeptIds="+this.checkDepGroup2.join(),
        headers: {
        ...
        data: {
					...
					remark: this.remark,
          upTime: this.upTime,
          // mainDeptIds: this.checkDepGroup1.join(),
          // duplicateDeptIds: this.checkDepGroup2.join(),
        }
      })
        .then(res => {
        ...
        .then(res => {
          ...
          if (res.data.resultCode == 200){
						this.units1 = res.data.data.values.slice(0,10);
